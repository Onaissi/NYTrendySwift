...
open class UISideMenuNavigationController: UINavigationController {
    ...
    //BY AOnaissi
    open var supportingRTL: Bool = true {
        didSet {
            if locked  {
                print("SideMenu Warning: a menu's RTL property cannot be changed after it has loaded.")
                supportingRTL = oldValue
            }
            if supportingRTL {
                leftSide = Locale.preferredLanguages.first ?? "en" == "en"
            }
        }
    }
